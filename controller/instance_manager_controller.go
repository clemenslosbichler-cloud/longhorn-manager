...
func (imc *InstanceManagerController) createInstanceManagerPodSpec(im *longhorn.InstanceManager, tolerations []corev1.Toleration, registrySecret string, nodeSelector map[string]string, dataEngine longhorn.DataEngineType) (*corev1.Pod, error) {
	...
	dataPath, err := imc.ds.GetDefaultDataPath()
	if err != nil {
		return nil, errors.Wrapf(err, "failed to get %v setting", types.SettingNameDefaultDataPath)
	}
	...
	podSpec.Spec.Containers[0].VolumeMounts = []corev1.VolumeMount{
		...
		{
			MountPath: filepath.Join(types.ReplicaHostPrefix, strings.TrimLeft(filepath.Join(controlPath, types.UnixDomainSocketDirectorySubpath), string(filepath.Separator))),
	...
	podSpec.Spec.Volumes = []corev1.Volume{
		...
		},
		{
			...
			VolumeSource: corev1.VolumeSource{
				HostPath: &corev1.HostPathVolumeSource{
					Path: filepath.Join(controlPath, types.EngineBinaryDirectorySubpath),
				},
			},
		},
		{
			...
			VolumeSource: corev1.VolumeSource{
				HostPath: &corev1.HostPathVolumeSource{
					Path: filepath.Join(controlPath, types.MetadataDirectorySubpath),
		...
		},
		{
			...
			VolumeSource: corev1.VolumeSource{
				HostPath: &corev1.HostPathVolumeSource{
					Path: filepath.Join(controlPath, types.UnixDomainSocketDirectorySubpath),
				},
			},
		},
		{
